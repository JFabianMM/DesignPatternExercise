...
AppModel.prototype.getSaveHistory = function getSaveHistory(){        
        setTimeout(()=>{
            let notes= history.state;              // Get the notes from the state
            if (notes!=null){
                this.saveNotes(notes);                 // Save the notes obtained from the state
                appController.renderHistoryState();
            }
        },10);
};
...
AppView.prototype.renderAddButton = function renderAddButton() {                           /// METHOD 
    ...
    addNoteButton.addEventListener("click", () => {                 // Added  to the Add Note Button 
        ...
        appController.addHistoryState();                            // Add a new note                 
   });
};

AppView.prototype.createNoteElements = function createNoteElements(id, content, creationDate, dateUpdate, elementReplace=0){
    let clone= this.createInsertNotesElements(id, content, creationDate, dateUpdate);
    let divBox = clone.querySelector("#box");
    let noteElement = clone.querySelector("textarea");
    ...
    if (elementReplace==0){
        ...
        this.renderElements(clone, notesContainer, elementReplace, divBox);
    }
    this.subscribeObserver(noteElement);                            // Subscribe the observer
}
...
AppView.prototype.handleDrop = function handleDrop(e) {    
    ...
    if (dragSrcEl != this) {                   
        ...
        appController.updateNotePosition(id1, id2)   // Interchange the information between the two notes in the local storage
        let eleOrigin=dragSrcEl;
        let eleDest=this;
        let notes = appController.appModel.getNotes();
        let targetNoteOrigin = notes.filter((note) => note.id == id1)[0];      // Get the note object to be updated
        let targetNoteDest = notes.filter((note) => note.id == id2)[0];      // Get the note object to be updated

        appView.unsubscribeObserver(eleOrigin.children[0]);  
        appView.unsubscribeObserver(eleDest.children[0]); 

        appView.createNoteElements(targetNoteOrigin.id, targetNoteOrigin.content, targetNoteOrigin.creationDate, targetNoteOrigin.dateUpdate, eleDest);
        appView.createNoteElements(targetNoteDest.id, targetNoteDest.content, targetNoteDest.creationDate, targetNoteDest.dateUpdate, eleOrigin);
         
        appView.removeElements(eleOrigin);
        appView.removeElements(eleDest);
    }
    appController.addHistoryState();
}
...
AppController.prototype.updateNotePosition = function updateNotePosition(id1, id2) {   // Interchange the information between the two notes in the local storage 
...
AppController.prototype.addHistoryState = function addHistoryState() {
    let notes = this.appModel.getNotes();
    history.pushState(notes, '1', null);     // Add a new history state
}

AppController.prototype.renderHistoryState = function renderHistoryState() {
    let elementCollection = document.getElementsByClassName("box");
    let len=elementCollection.length;
    for (let i=0;i<len;i++){
        this.appView.removeElements(elementCollection[0]); 
    }
    textSubject.observers=[];
    this.render();
}
